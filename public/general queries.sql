...
SELECT u.center_code, u.name
FROM users u
WHERE NOT EXISTS (
    SELECT 1 FROM exam e WHERE e.user_id = u.center_code AND e.user_name = u.name
) OR NOT EXISTS (
    SELECT 1 FROM homework h WHERE h.user_id = u.center_code AND h.user_name = u.name
);
